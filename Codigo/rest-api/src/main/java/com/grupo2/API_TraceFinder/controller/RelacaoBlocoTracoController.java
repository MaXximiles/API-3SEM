...
public class RelacaoBlocoTracoController {
	...
	public List<RelacaoBlocoTracoRs> selectAll()
	{
		...
		return RelArqTraco.stream().map((relArqTraco) -> RelacaoBlocoTracoRs.converter(relArqTraco)).collect(Collectors.toList());	
	}
	...
	// UPDATE
	@PutMapping("/{id}")
	public void updateRelacaoBlocoTraco(@PathVariable Long id, @RequestBody RelacaoBlocoTracoRq relBlocoTraco) throws Exception
	{
