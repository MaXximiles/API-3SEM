...
public class BlocoCustomRepository {
  ...
  public BlocoCustomRepository(EntityManager em) {
    this.em = em;
  }

  public List<Bloco> SelectJoinDocumento(Long blocoid, String blocosecao, String blocosubsecao, String bloconomebloco,
      String blococodebloco, String blococaminho) {
    // O nome da tabela tem q ser o nome da classe para mapear os parametros
    // crie um alias (Documento AS D) para dizer que esta selecionando a classe ao
    // invés da tabela
    ...
    /*
     * String query = "SELECT C FROM Codelist AS C "; query +=
     * " JOIN C.Documento D "; query += " WHERE D.documento_id = 1 "; String
     * condicao = " WHERE ";
     */

    /*
     * if(documentonome != null) { query += condicao +
     * " documento_nome = :documentonome"; condicao = " AND "; } if(documentopn !=
     * null) { query += condicao + " documento_pn = :documentopn"; }
     */
     ...
     * if(documentocaminho != null){sql.setParameter("documento_caminho",
     * documentocaminho );}
     */
